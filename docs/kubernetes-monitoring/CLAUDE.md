...
> **2026-09-09 — 자료가 `study/`로 이동했다.**
> 이 디렉터리의 `docs/`·`lectures/`·`slides/`·`labs/`는 저장소 루트의 `study/`로
> 합쳐졌다. 강의안의 이론은 각 장 01절, 실습서는 02~11절, 실습 코드는 `study/labs/`다.
> `docs/k8s-observability-study/`는 이 디렉터리 `docs/`의 복제본이었고 함께 정리했다.
> 지금 여기 남은 것은 **`analysis/` 세 편(정정 목록의 정본)과 원전 PDF**뿐이다.
> 아래 규약 중 문체·버전·정정 목록은 여전히 유효하고, 경로와 디렉터리 구조 설명은
> `study/README.md`와 루트 `CLAUDE.md`를 본다.
...
아키텍처·UML·시퀀스 등 다이어그램 신규 제작은 **archify 스킬**을 쓰던 규약이다.

> **2026-09-09 확인 — archify 가 `~/.claude/skills/` 에 없다.** `slides/figures/` 의
> 입력 JSON 5종은 `study/ppt/figures/` 로 옮겨 두었지만 지금은 재생성할 수 없다.
> 도판을 새로 만들어야 하면 `course/figures/` 가 쓰는 손으로 그린 인라인 SVG 방식을
> 따른다 — 흰 배경, 회색 선, 모노스페이스 라벨.
